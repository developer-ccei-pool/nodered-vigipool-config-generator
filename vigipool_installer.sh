...
link_to_scripts="https://raw.githubusercontent.com/developer-ccei-pool/nodered-vigipool-config-generator/main/"
ip_address_mqtt_debug="192.168.1.123"
debug=false
...
echo ""
echo "----- Node-Red Vigipool Config Generator -----"
echo ""
...
while true; do
    ...
    if [ "$menu_choice" != "${menu_choice#[1enEN]}" ] ;then 
        ...
        text_step_2="Step 2.1 - Search for the VigiPool product in your network"
        
        text_step_2_2="Step 2.2 - Search for the MQTT Broker in your network"
        ...
        text_step_3="Step 3 - Selecting the Node-Red stream format"
        text_want="In what format do you want your configuration ?"
        text_want_auto="All Vigipool products in one flow"
        text_want_manu="A flow for each Vigipool product"
        ...
        text_step_4="Step 4 – Adapting the file according to your configuration"
        ...
        text_step_2="Étape 2.1 - Recherche du produit VigiPool dans votre réseau"
        
        text_step_2_2="Étape 2.2 - Recherche du Broker MQTT dans votre réseau"
        ...
        text_step_3="Étape 3 - Sélection du format du flux Node-Red"
        text_want="Dans quel format voulez-vous votre configuration ?"
        text_want_auto="Tous les produits Vigipool dans un flux"
        text_want_manu="Un flux pour chaque produit Vigipool"
        ...
        text_step_4="Étape 4 - Adaptation du fichier en fonction de votre configuration"
...
while [[ "$ip_address_broker" == "" ]]; do
    echo "$text_do";
    echo "1 - $text_automatic";
    echo "2 - $text_manual";
    echo "$text_please : "
    if [[ $debug == true ]]; then
        ip_address_broker="192.168.1.123"
        menu_choice=""
    else
        read menu_choice
    fi
            ...
            if [[ "$cat_grep" == "0" ]]; then
                ...
                while true; do
                    ...
                    if [ "$menu_choice" != "${menu_choice#[1]}" ] ;then 
                        # 1 - Use this IP address
                        echo "$text_you '$text_what_1'"
                        ip_address_broker=${ip_address_to_test[i]}
                        break
                    elif [ "$menu_choice" != "${menu_choice#[2]}" ] ;then 
                        # 2 - Continue scanning
                        echo "$text_you '$text_what_2'"
...
while true; do
    echo "$text_want"
    echo "1 - $text_want_auto";
    echo "2 - $text_want_manu";
    echo "$text_please : "
    read menu_choice
    if [ "$menu_choice" != "${menu_choice#[1aA]}" ] ;then 
        # 1 - Automatic
        echo "$text_you '$text_want_auto'"
        cp nodered_templates/nodered_one_flux.json ./nodered_vigipool.json
        break
    elif [ "$menu_choice" != "${menu_choice#[2mM]}" ] ;then 
        # 2 - Manual
        echo "$text_you '$text_want_manu'"
        cp nodered_templates/nodered_multi_flux.json ./nodered_vigipool.json
        break
    else
        echo The choice was not understood, please try again
    fi
...
replace_template_name() {
    # if [[ -z "$2" ]]; then
        # echo ""
    if [[ -n "$2" ]]; then
        sed -i "s/${1}_XXXXXXXXXXXX/$2/" ./nodered_vigipool.json
        # echo "replacing ${1}_XXXXXXXXXXXX with $2"
    fi
}
